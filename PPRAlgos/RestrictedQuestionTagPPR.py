...
#
#
# with open(os.path.join(DATA_DIR, 'TT_communities.map'), 'r') as f:
#     for line in f.readlines():
#         k,v = line.strip().split(',')
#         QT_COMMUNITIES[k] = v
...
def get_random_question_two(G, tag_node, initial_prob, initial_node):
    ...
    neighbor_questions = list(G[tag_node])
    # start = time.time()
    if len(neighbor_questions) >= 50000:
...
# def restrict_graph(G, node):
#     strongest_tags = sorted(G[node], key=operator.itemgetter(1), reverse=True)[0:2]
#     communities = [QT_COMMUNITIES[x] for x in strongest_tags]
#     subgraph = G.subgraph(filter(lambda x: QT_COMMUNITIES[x] in communities, G))
